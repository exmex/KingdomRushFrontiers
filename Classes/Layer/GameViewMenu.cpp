...
void GameViewMenu::onEnterTransitionDidFinish()
{
	...
	starLabel->setString(__String::createWithFormat("%d/65", UserDefault::getInstance()->getIntegerForKey(instance->SLOTX_STAR,0))->getCString());
	diamondsLabel->setString(__String::createWithFormat("%d", UserDefault::getInstance()->getIntegerForKey(instance->SLOTX_GEM,0))->getCString());
	if(isFirst){
