...
bool BaseBuildIcon::initIcon(int type,int money)
{
	...
	isSelected = false;
	auto test = __String::createWithFormat("main_icons_00%02d.png",type)->getCString();
	baseIcon = Sprite::createWithSpriteFrameName(__String::createWithFormat("main_icons_00%02d.png",type)->getCString());
	addChild(baseIcon);
	priceIcon = Sprite::createWithSpriteFrameName("price_tag.png");
	priceIcon->setPosition(Point(0,-baseIcon->getContentSize().height/2));
	addChild(priceIcon);
	priceLabel = Label::createWithTTF(__String::createWithFormat("%d",money)->getCString(),"arial.ttf",20);
...
{
	setMoney(money);
	priceLabel->setString(__String::createWithFormat("%d",money)->getCString());
}
...
void BaseBuildIcon::setNotEnoughMoney()
{
	...
	baseIcon->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(__String::createWithFormat("main_icons_disabled_00%02d.png",type)->getCString()));
...
void BaseBuildIcon::setEnoughMoney()
{
	...
	baseIcon->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(__String::createWithFormat("main_icons_00%02d.png",type)->getCString()));
