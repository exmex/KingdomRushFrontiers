...
bool ShopIcon::initIcon(int type)
{
	...
	this->type = type;
	icon = Sprite::createWithSpriteFrameName(__String::createWithFormat("inaps_Icons_000%d.png",type)->getCString());
	addChild(icon);
	glow = Sprite::createWithSpriteFrameName(__String::createWithFormat("inaps_IconsGlow_000%d.png",type)->getCString());
	glow->setScale(2.0f);
	addChild(glow);
	setNotGlowing();
